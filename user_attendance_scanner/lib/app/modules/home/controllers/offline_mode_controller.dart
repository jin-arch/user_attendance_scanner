...
import 'package:user_attendance_scanner/app/data/services/inactivity_timer_service.dart';
...
class OfflineModeController extends GetxController {
  ...
  void setSiteId(String siteId) {
    ...
    if (Get.isRegistered<PendingUploadService>()) {
      Get.find<PendingUploadService>().notifySiteChanged(siteId);
    }
  }
  ...
  Future<void> syncAllPendingNow() async {
    ...
    // Always flush all pending records when coming online from offline
    // Don't limit to specific site to ensure all offline records are uploaded
    await Get.find<PendingUploadService>().flushAllPending(
      siteId: null, // Flush all sites to ensure no pending records are missed
      reason: 'offline_mode_controller',
    );
  }
