...
class LogsController extends GetxController {
  ...
  // API Configuration — see [ApiConfig] / [HrisEndpoints] in app/data/api/hris_api.dart
  ...
  /// True only while the Time Logs page controller is active (not home/dashboard).
  bool _timeLogsScanEnabled = false;

  LogsController({this.siteId});

  bool get _canRunTimeLogsScan => _timeLogsScanEnabled && !isClosed;
  ...
  void onInit() {
    super.onInit();
    _timeLogsScanEnabled = true;
    _deviceService = Get.find<DeviceService>();
    _employeeRepository = Get.find<EmployeeRepository>();
    _initializeDevice();
    // Fingerprint identification for time logs runs only on this page.
  ...
  @override
  void onClose() {
    _timeLogsScanEnabled = false;
    unawaited(_stopTimeLogsScanner());
    try {
  ...
  Future<void> _stopTimeLogsScanner() async {
    try {
      await _deviceService.stopScanningMode();
    } catch (e) {
      debugPrint('[LOGS_CONTROLLER] stopScanningMode on close: $e');
    }
  ...
  Future<List<Map<String, dynamic>>> _fetchLogsFromApi(String siteId) async {
    try {
      ...
      final headers = {
        ...
        'Authorization': 'Basic ${base64Encode(utf8.encode('${ApiConfig.username}:${ApiConfig.password}'))}',
      };
  ...
  Future<void> sendLogToServer(Map<String, dynamic> log) async {
    try {
      ...
      final url = HrisEndpoints.uri(HrisEndpoints.postTimelog);
