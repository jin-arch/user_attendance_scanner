...
/// Result of validation before opening the saving dialog.
enum EnrollmentSavePrecheck {
  ok,
  missingEmployeeDetails,
  incompleteScans,
  alreadyEnrolled,
  cannotVerifyFingerprint,
}
...
class EnrollmentController extends GetxController {
  ...
  static const int minTimeBetweenScansMs = 400;
  ...
  VoidCallback? onFingerprintAlreadyEnrolled;
  ...
  void _onFormChanged() {
    ...
    // Removed automatic photo loading - photo should only load after successful lookup
  }
  ...
  void _prefillEmployeeDetails() {
    ...
    showForm.value = true;
    // Don't automatically lookup employee - wait for user to press LOOKUP or SCAN button
    if ((employeeId != null && employeeId!.isNotEmpty) ||
        (employeeName != null && employeeName!.isNotEmpty)) {
      _clearScanState();
    }
    // Clear profile details to prevent showing previous employee data
  ...
  Future<void> _loadEmployeePhotoForId(String empId) async {
    try {
      ...
      if (!await LocalDb.hasEmployeePhoto(
        ...
        // Don't clear the photo if user just took one
        // Only clear if there's no current photo
        if (selfieImageBytes.value == null) {
          selfieImageBytes.value = null;
        }
        return;
      ...
      onCapture: (bytes) async {
        ...
        if (empId.isNotEmpty) {
          try {
            ...
            onSuccess?.call('Profile photo saved automatically');
          } catch (e) {
            debugPrint('Error auto-saving photo: $e');
  ...
  /// Validates save prerequisites without opening the saving UI.
  Future<EnrollmentSavePrecheck> precheckEnrollmentSave() async {
    if (idController.text.trim().isEmpty || usernameController.text.trim().isEmpty) {
      return EnrollmentSavePrecheck.missingEmployeeDetails;
    }

    if (leftThumbScans.value < scansPerFinger ||
        rightThumbScans.value < scansPerFinger) {
      return EnrollmentSavePrecheck.incompleteScans;
    }

    if (!isEditMode) {
      final site = await _activeSiteId();
      if (await _employeeHasLocalFingerprint(
        employeeId: idController.text.trim(),
        siteId: site,
      )) {
        return EnrollmentSavePrecheck.alreadyEnrolled;
      }
      ...
      if (apiHas == null) {
        return EnrollmentSavePrecheck.cannotVerifyFingerprint;
      }
      if (apiHas) {
        return EnrollmentSavePrecheck.alreadyEnrolled;
      }
    ...
    return EnrollmentSavePrecheck.ok;
  }

  String messageForSavePrecheck(EnrollmentSavePrecheck result) {
    switch (result) {
      case EnrollmentSavePrecheck.ok:
        return '';
      case EnrollmentSavePrecheck.missingEmployeeDetails:
        return 'Please look up the employee (ID and name) before saving.';
      case EnrollmentSavePrecheck.incompleteScans:
        return 'Please scan each thumb 3 times (3 left, 3 right).';
      case EnrollmentSavePrecheck.alreadyEnrolled:
        return 'This employee already has enrolled fingerprints. '
            'Use Update Fingerprint only when replacing an existing enrollment.';
      case EnrollmentSavePrecheck.cannotVerifyFingerprint:
        return 'Unable to verify fingerprint status. Please sync online and try again.';
    }
  }
  ...
  Future<EnrollmentSaveContext?> saveEnrollmentLocal() async {
    ...
    final precheck = await precheckEnrollmentSave();
    if (precheck != EnrollmentSavePrecheck.ok) {
      onError?.call(messageForSavePrecheck(precheck));
      return null;
    }
  ...
  bool _setFingerprintEnrollmentBlocked(String reason) {
    ...
    if (reason.toLowerCase().contains('already has enrolled')) {
      onFingerprintAlreadyEnrolled?.call();
    } else {
      onError?.call(reason);
    }
    _stopScanLoop();
  ...
  Future<bool> _ensureFingerprintEnrollmentAllowed(String employeeId) async {
    ...
    // Update flow replaces existing templates; only block brand-new enrollments.
  ...
  Future<bool> loadEmployeeDetails(String employeeId) async {
    ...
    try {
      ...
      if (!isEditMode) {
        final allowed = await _ensureFingerprintEnrollmentAllowed(
      ...
      // Automatically start enrollment scanning after lookup
      if (deviceInitialized.value && fingerprintEnrollmentAllowed.value) {
        _startScanLoop();
      }
